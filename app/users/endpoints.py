...
async def user_register(user : UserSchema = fastapi.Body(default=None)):
    user.password = AuthHandler.get_password_hash(user.password) # Encrypt the password
...
async def user_login(user_login_schema : UserLoginSchema = fastapi.Body(default=None)):
    ...
    if (user is None) or (not AuthHandler.verify_password(user_login_schema.password, user["password"])):
        raise fastapi.HTTPException(status_code=401, detail="Invalid email and/or password")
    token = AuthHandler.signJWT(user["email"]) # Encrypt the token
    return {'token': token}
