...
def item_exists(company_id: str, item_id: str):
    if inventory_db.get_item_by_id(company_id=company_id, item_id=item_id) is None:
        raise HTTPException(status_code=404, detail="Item not found")

def inventory_exists(company_id: str, inventory_id: str):
    if inventory_db.get_inventory_by_id(company_id=company_id, inventory_id=inventory_id) is None:
