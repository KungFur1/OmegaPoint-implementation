...
@handle_db_error
async def get_item_discounts(item_id: str):
    return {"data": discount_db.get_item_discounts(item_id=item_id)}


@router.post("/cinematic/items/{item_id}/discounts", tags=["discounts"], status_code=201)
@handle_db_error
async def create_item_discount(item_id: str, item_discount_data: ItemDiscountCreateModel = fastapi.Body(default=None),
                               user_identification: UserIdentification = fastapi.Depends(authorization_wrapper)):
    
    discount_db.post_item_discount(item_id=item_id, item_discount_data=item_discount_data)
...
@handle_db_error
async def edit_item_discount(discount_id: str, item_discount_data: ItemDiscountCreateModel = fastapi.Body(default=None),
                             user_identification: UserIdentification = fastapi.Depends(authorization_wrapper)):
    
    discount_db.put_item_discount(discount_id=discount_id, item_discount_data=item_discount_data)
...
@handle_db_error
async def delete_item_discount(discount_id: str,
                               user_identification: UserIdentification = fastapi.Depends(authorization_wrapper)):
    
    discount_db.delete_item_discount(discount_id=discount_id)
