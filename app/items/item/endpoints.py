...
async def create_item(item_create_data: ItemCreateModel = fastapi.Body(default=None), user_identification: UserIdentification = fastapi.Depends(authorization_wrapper)):
    ...
    item_db.post_item(item_create_data=item_create_data)
    return {"info": "item successfully inserted"}


@router.put("/cinematic/items/{item_id}", tags=["items"], status_code=200)
@handle_db_error
async def edit_item(item_id: str, item_update_data: ItemUpdateModel = fastapi.Body(default=None),
                    user_identification: UserIdentification = fastapi.Depends(authorization_wrapper)):
    users_check.is_admin(user_id=user_identification.id)

    item_db.put_item(item_id=item_id, item_update_data=item_update_data)
...
async def delete_item(item_id: str,
    ...
    item_db.delete_item(item_id=item_id)
